...
public class RAR_Right : MonoBehaviour
{
    [System.Serializable]
    ...
    void Update()
    {
        ...
        if (rayInteractor.TryGetCurrent3DRaycastHit(out RaycastHit hit))
        {
            ...
            bool isRight = localDirection.x > 0.1f;
                ...
                {
                    if (isLeft) AdjustBlendshape(Rock,Land_1,Land_2,Land_3, true);
                    if (isRight) AdjustBlendshape(Rock,Land_1,Land_2,Land_3, false);
                }
        }
    ...
    private void AdjustBlendshape(TargetModel_1 Rock,TargetModel_1 Land_1,TargetModel_2 Land_2,TargetModel_3 Land_3, bool isLeft)
    {
        if (isLeft) // **向左調整 (聚合)**
        {
            ...
            else if (!sharedState.LeftBlow_L3Active && !sharedState.LeftUp_L3Active 
            ...
            else if(!sharedState.LeftBlow_L2Active && !sharedState.LeftUp_L2Active && !sharedState.LeftUp_L1Active&& !
            sharedState.LeftUp_RockActive && Land_2.currentBlow == 0 && Land_2.currentUp == 0 && Land_1.currentUp == 0
        ...
        else // **向右調整 (張裂)**
        {
            ...
            else if (!sharedState.LeftClose_L1Active && !sharedState.LeftClose_RockActive 
            ...
            else if(sharedState.LeftBlow_L2isActive && sharedState.LeftUp_L2isActive && sharedState.LeftUp_L1isActive&& sharedState.LeftUp_RockisActive && Land_2.currentBlow == 100
        ...
        // **更新共享狀態**
        //Rock
        sharedState.RightClose_RockActive = Rock.currentClose > 0;  // RAR_Right 的 close 是否未歸零
        sharedState.RightUp_RockActive = Rock.currentUp > 0; // RAR_Right 的 up 是否未歸零
        //Land_1
        sharedState.RightClose_L1Active = Land_1.currentClose > 0;  // RAR_Right 的 close 是否未歸零
        sharedState.RightUp_L1Active = Land_1.currentUp > 0; // RAR_Right 的 up 是否未歸零
        //Land_2
        sharedState.RightUp_L2Active = Land_2.currentUp > 0; // RAR_Right 的 up 是否未歸零
        sharedState.RightBlow_L2Active = Land_2.currentBlow > 0; // RAR_Right 的 blow 是否未歸零
        //Land_3
        sharedState.RightUp_L3Active = Land_3.currentUp_1 > 0; // RAR_Right 的 up_1 是否未歸零
        sharedState.RightBlow_L3Active = Land_3.currentBlow_1 > 0; // RAR_Right 的 blow_1 是否未歸零
        //張裂
        sharedState.RightUp_RockisActive = Rock.currentUp == 100; // RAR_Right 的 up 是否100
        sharedState.RightUp_L1isActive = Land_1.currentUp == 100; // RAR_Right 的 up 是否100
        sharedState.RightUp_L2isActive = Land_2.currentUp == 100; // RAR_Right 的 up 是否100
        sharedState.RightBlow_L2isActive = Land_2.currentBlow == 100; // RAR_Right 的 blow 是否100
        ...
        if(Rock.currentClose == 100 && Land_1.currentClose == 100)
            sharedState.RightClose = true;
        else if(Rock.currentClose == 99 && Land_1.currentClose == 99)
            sharedState.RightClose = false;
    }
}
