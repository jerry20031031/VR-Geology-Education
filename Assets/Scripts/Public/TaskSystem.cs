...
{
    [Header("UI 設置")]
    public TMP_Text currentTaskText; // TextMeshPro 元件，用於顯示當前任務名稱

    [Header("任務邏輯")]
    public List<GameObject> availableObjects; // 可選的判斷物件列表
    public List<Task> tasks; // 任務列表
    public List<TaskGroup> taskGroups; // 任務組
    public int currentTaskID; // 當前執行任務的編號
    public List<Task> CurrentTask;
    ...
    private void Update()
    {
        ...
        if (taskNeedsCheck)
        {
            ...
            Debug.Log($"任務數量: {CurrentTaskCount}");
            ...
            Debug.Log($"已完成任務數量: {tasksToRemove.Count}");
    ...
    private void OnTaskGroupCompleted()
    {
        TaskGroup currentGroup = taskGroups.FirstOrDefault(group => group.groupID == currentTaskID);
        currentGroup?.OnGroupStart?.Invoke();
    }
    ...
    public bool SetCurrentTaskGroup(int taskID)
    {
        ...
        // 觸發當前任務組的開始事件
        TaskGroup group = taskGroups.FirstOrDefault(g => g.groupID == taskID);
    ...
    private void UpdateTaskTextUI()
    {
        if (currentTaskText != null)
        {
            ...
            {
                currentTaskText.text = "所有任務已完成！";
            }
            else
            {
                string taskNames = string.Join(", ", CurrentTask.Select(task => task.taskName));
                currentTaskText.text = $"{taskNames}";
            }
        }
    }
