...
namespace cherrydev
{
    public class DialogBehaviour : MonoBehaviour
    {
        ...
        public event Action<int, AnswerNode> OnAnswerButtonKeyPressed;

        private List<Node> historyNodes = new List<Node>();

        private bool isNew = true;

        public InputActionReference nextSentence;
        ...
        private void Update()
        {
            ...
            HandleAnswerButtonKeyInput();
            /*if (historyNodes.Count > 1)
            {
                Debug.Log(historyNodes.Count);
            }*/
        }
        public int GetHistoryCount()
        {
            return historyNodes.Count;
        }
        public void GoToPreviousNode()
        {
            if (historyNodes.Count > 1)
            {
                ...
                Debug.Log(historyNodes.Count);
                currentNode = historyNodes[historyNodes.Count - 1];
                isNew = false;
                HandleDialogGraphCurrentNode(currentNode);
            }
        }
        private void HandleAnswerButtonKeyInput()
        {
            AnswerNode answerNode = currentNode as AnswerNode;
            if (answerNode == null)
            {
                return;
            }
            ...
            if (Input.anyKeyDown)
            {
                int maxKeyIndex = Mathf.Min(answerNode.childSentenceNodes.Count, 9);
                for (int i = 1; i <= maxKeyIndex; i++)
                {
                    if (Input.GetKeyDown(KeyCode.Alpha0 + i))
                    {
                        OnAnswerButtonKeyPressed?.Invoke(i - 1, answerNode);
                        break;
                    }
                }
            }
        ...
        private void HandleDialogGraphCurrentNode(Node currentNode)
        {
            ...
            if (isNew) { historyNodes.Add(this.currentNode); };
            isNew = true;
        ...
        private void HandleSentenceNode(Node currentNode)
        {
            ...
            if (currentNode is SentenceNode sN)
            {
                // �p�G�O SentenceNode�A�N�i�H�ե� GetSentenceCharacterName()
                string NodeName = sentenceNode.GetSentenceCharacterName();
                if(voiceManager == null)
                {
                    Debug.LogWarning("VoiceManager is not assigned. Skipping voice playback.");
                    return;
                }
                voiceManager.PlayVoice(NodeName);
            }
            else
            {
                Debug.LogWarning("Current node is not a SentenceNode");
            }
        }
        ...
        private void WriteDialogText(string text)
        {
            ...
            {
                Debug.LogWarning("Cannot start 'WriteDialogText' coroutine because the GameObject is inactive.");
            }
        }
        ...
        private bool CheckNextSentenceKeyCodes()
        {
            ...
            { 
                if (Input.GetKeyDown(nextSentenceKeyCodes[i]) || nextSentence.action.triggered)
                {
                    return true;
                }
            }
